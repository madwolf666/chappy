...
def _read_learned_data():
    ...
    _loto7 = pd.read_csv('C:\\Users\\hal\\Downloads\\loto7.csv', sep=",", encoding='shift_jis')
    _loto7.head
...
def _set_value_1(h_num):
    ...
    #説明変数に"BONUS数字1"を利用
    _X = _loto7.loc[:, ['日付']].as_matrix()

    #目的変数に"第1数字"を利用
    _Y = _loto7['第' + str(h_num) +'数字'].as_matrix()

def _set_value(h_num):
    global _loto7, _X, _Y, _except_quality
    ...
    a_drop_col = ['開催回', '日付']
    _except_quality = _loto7.drop(a_drop_col, axis=1)
